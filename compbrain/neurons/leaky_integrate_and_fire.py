...
class LIFNeuron(BaseComponent):
    """Leaky Integrate and Fire Neuron Model"""

    def __init__(self, name, **kwargs):
        """
        initial function
        :param name: name for the leaky integrate and fire neuron, every neuron in the circuit
            should be different
        :param kwargs: keyword arguments that overwrite initial conditions of state
            variables and values of parameters
        """
        super(LIFNeuron, self).__init__(name, **kwargs)
        self.params: OrderedDict = OrderedDict(
            V_T=20.0, V_0=0, V_imp=20,
            C=1.0, R=1.0, dt=1e-5,
        )
    ...
    def compute(self, I_syn: float, I_ext: float, dt:float) -> dict:
        """
        Leaky Integrate and Fire gradient function
